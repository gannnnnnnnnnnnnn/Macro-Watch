      ...
      {safeItems.map((item, index) => (
        <div key={`${item.name ?? "indicator"}-${index}`} className="flex items-center justify-between gap-4 border-b border-line pb-3 last:border-0 last:pb-0">
          <div>
            ...
            {typeof item.delta === "number" ? <p className="mt-1 text-xs text-slate-500">Last obs {formatDelta(item.delta, item.unit ?? "")} · {typeof item.one_year_delta === "number" ? `1Y ${formatDelta(item.one_year_delta, item.unit ?? "")}` : "context only"}</p> : item.delta_label ? <p className="mt-1 text-xs text-slate-500">{item.delta_label}</p> : null}
          </div>
